@php
    ...
    // Example: "Donate to Jalal Unus - Pabna-1"
    $title = $constituencyName
        ? 'Donate to ' . $name . ' - ' . $constituencyName
        : 'Donate to ' . $name;

    $description = 'Scan the QR code to donate and support ' . $name . '.';
    $image = $candidate->image ?? null;
    $baseUrl = $baseUrl ?? (config('app.url') ?? url('/'));
