...
export default function CandidateManifestoPage() {
  ...
  }, [candidateId, pagination.page, pagination.per_page, search, categoryFilter, statusFilter]);
  ...
  const fetchManifestos = async () => {
    setLoading(true);
    try {
      const response = await api.get(`/candidates/${candidateId}/manifestos`);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/candidates/${candidateId}/manifestos/${id}`);
      fetchManifestos();
      ...
      alert('Failed to delete manifesto');
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      // Delete all selected manifestos
      const deletePromises = Array.from(selectedItems).map((id) =>
        api.delete(`/candidates/${candidateId}/manifestos/${id}`)
      );
      ...
      alert('Failed to delete some manifestos');
  ...
  const handleLike = async (manifesto) => {
    try {
      ...
      // TODO: Add API endpoint for like if needed
    } catch (error) {
  ...
  const handleDislike = async (manifesto) => {
    try {
      ...
      // TODO: Add API endpoint for dislike if needed
    } catch (error) {
  ...
  const onSubmit = async (data) => {
    try {
      if (editingManifesto) {
        await api.put(
          `/candidates/${candidateId}/manifestos/${editingManifesto.id}`,
          data
        );
      } else {
        await api.post(`/candidates/${candidateId}/manifestos`, data);
      }
  ...
  const filteredAndSortedManifestos = useMemo(() => {
    ...
    if (categoryFilter) {
      filtered = filtered.filter(
        (m) => String(m.category_id) === categoryFilter
      );
    }
  ...
  const isAllSelected =
    ...
    selectedItems.size === paginatedManifestos.length;
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <DialogFooter>
                  ...
                  <Button type="submit">
                    {editingManifesto ? 'Update' : 'Create'}
                  </Button>
                </DialogFooter>
              </form>
            </Form>
