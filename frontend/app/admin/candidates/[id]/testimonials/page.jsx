...
const DropzoneInput = ({ label, field, fileUrl, onFile, hint, heightClass = 'h-32', accept = 'image/*' }) => {
  ...
  return (
    <div className="space-y-2">
      ...
      <div
        ...
        className={`border border-dashed rounded-md bg-muted/20 flex items-center justify-center text-sm text-muted-foreground cursor-pointer ${heightClass} relative overflow-hidden hover:bg-muted/30 transition-colors`}
      >
        {fileUrl ? (
...
export default function CandidateTestimonialsPage() {
  ...
  const handleFileUpload = (field, file, preview) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      } else if (data.avatar_url && typeof data.avatar_url === 'string') {
        formData.append('avatar_url', data.avatar_url);
      }
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormControl>
                          <DropzoneInput
                            label="Photo"
                            hint="Recommended: landscape"
                            field="photo_url"
                            fileUrl={photoPreview || (typeof field.value === 'string' ? field.value : '')}
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormControl>
                          <DropzoneInput
                            label="Avatar"
                            hint="Recommended: square"
                            field="avatar_url"
                            fileUrl={avatarPreview || (typeof field.value === 'string' ? field.value : '')}
                            onFile={handleFileUpload}
                            heightClass="h-32"
                          />
                        </FormControl>
